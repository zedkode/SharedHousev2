...
A payment checkbox means a member recorded a payment. Use confirmation/dispute based on actual household evidence. Correct mistakes through reversal/adjustment with a reason. Never use SharedHouse as the sole proof in a legal dispute; retain appropriate bank/supplier records.

## 6. Build a fair chore plan

Create clear templates for indoor cleaning, outdoor/garden work, bins out, bins returned and restocking. Set realistic duration/weight, recurrence, proof requirement and postpone window. Use balanced or round-robin assignment and review fairness without exposing private exemption reasons.
